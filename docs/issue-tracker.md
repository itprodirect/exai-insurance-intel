...
`Local #24` captures the Exa 2026 modernization cleanup completed before the cost/grounding slice. Cost estimates now use centralized, overrideable pricing assumptions; remaining follow-up should focus on modern grounding metadata, not on restoring deprecated payload fields.
